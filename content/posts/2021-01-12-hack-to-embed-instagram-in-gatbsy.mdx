...
There were a couple of problems if you try to do that. Well, technically they are not problems, they are more
contrains given that Instagram requires you to create and app for you to be able to use their API. Moreover,
...
intergrations, e.i. using Zappier to triger your CDN to built.

There is a workaround, that right now, seems easy and quicker to set up. 

# Instagram hidden API

You might not have noticed, but if you load instagram on the webrowser there are inoumerous network request that 
are basically pinging Instagram graphQL APIs so that your webrowser can reander them on the DOM. 
...
Now, using that newly created component, I can request my 6 Instagram lastest posts to be rendered here, or anywhere on my digital garden!
...
I am temped to create a `gatsby-plugin` for this... 
